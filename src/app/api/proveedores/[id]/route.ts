...
/** DELETE /api/proveedores/:id — borra el proveedor (bloquea si tiene compras). */
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      if (code === "23503") {
        return NextResponse.json(
          errorResponse("El proveedor tiene compras u órdenes asociadas. Marcalo como inactivo en vez de borrarlo para conservar el histórico."),
          { status: 409 }
        );
      }
      throw e;
